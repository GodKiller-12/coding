#include <bits/stdc++.h>
using namespace std;
#define int long long
#define f first
#define s second

class segtree
{
private:
    vector<pair<int, int>> tree;
    vector<int> arr;
    int n;
    void build(int ind, int left, int right)
    {
        if (left == right)
        {
            tree[ind].f = max(arr[left], 0ll);
            tree[ind].s = arr[left];
            return;
        }
        //  first me max prefix sum
        //  second me total sum
        int mid = (left + right) / 2;
        build(2 * ind + 1, left, mid);
        build(2 * ind + 2, mid + 1, right);
        tree[ind].f = max(tree[2 * ind + 1].s + tree[2 * ind + 2].f, tree[2 * ind + 1].f);
        tree[ind].s = tree[2 * ind + 1].s + tree[2 * ind + 2].s;
    }

    pair<int, int> query(int ind, int l, int r, int ql, int qr)
    {
        //    no overlap
        if (ql > r || l > qr)
            return {0, 0};
        //   complete overlap

        if (ql <= l && qr >= r)
            return tree[ind];

        int mid = (l + r) / 2;
        pair<int, int> lq = query(2 * ind + 1, l, mid, ql, qr);
        pair<int, int> rq = query(2 * ind + 2, mid + 1, r, ql, qr);
        pair<int, int> ans;
        ans.f = max(lq.s + rq.f, lq.f);
        ans.s = lq.s + rq.s;
        return ans;
    }

    void update(int ind, int l, int r, int pos, int val)
    {
        if (l == r)
        {
            arr[pos] = val;
            tree[ind].s = val;
            tree[ind].f = max(val, 0ll);
            return;
        }

        int mid = (l + r) / 2;
        if (pos <= mid)
        {
            update(2 * ind + 1, l, mid, pos, val);
        }
        else
        {
            update(2 * ind + 2, mid + 1, r, pos, val);
        }

        tree[ind].s = tree[2 * ind + 1].s + tree[2 * ind + 2].s;
        tree[ind].f = max(tree[2 * ind + 1].s + tree[2 * ind + 2].f, tree[2 * ind + 1].f);
    }

public:
    segtree(vector<int> &a)
    {
        n = a.size();
        arr = a;
        tree.resize(4 * n);
        build(0, 0, n - 1);
    }

    pair<int, int> getsum(int l, int r)
    {
        return query(0, 0, n - 1, l, r);
    }

    void upd(int pos, int val)
    {
        return update(0, 0, n - 1, pos, val);
    }
};

signed main()
{
    int n, m;
    cin >> n >> m;
    vector<int> a(n);
    for (int i = 0; i < n; i++)
    {
        cin >> a[i];
    }

    segtree st(a);

    while (m--)
    {
        int x;
        cin >> x;
        if (x == 1)
        {
            int pos, val;
            cin >> pos >> val;
            pos--;
            st.upd(pos, val);
        }
        else
        {
            int l, r;
            cin >> l >> r;
            l--;
            r--;
            cout << st.getsum(l, r).f << endl;
        }
    }
}